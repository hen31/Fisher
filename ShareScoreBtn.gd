...
func _on_ShareScoreBtn_pressed():
	if share != null:
		var gamestate = get_node("/root/GameState")
		share.shareText(tr('shareTitle'), tr('shareScore'), tr('shareScoreMessage') % [gamestate.LastScore, gamestate.LastSeedHex])
